#!/usr/bin/env bash
#
# Release Himekuri via Amore (+ optionally a GitHub release, once the
# repo exists and is public).
#
# Usage:
#   scripts/release.sh                    Use version + build from Version.xcconfig
#   scripts/release.sh --version 0.1.1    Set marketing version (and bump build)
#   scripts/release.sh --version 0.1.1 --build 7
...
#
# The GitHub steps are skipped automatically while this project has no git
# repository / remote — remove nothing when open-sourcing, it just starts
# working.
...
if in_git_repo; then
    ...
    echo "  ⚠ no git repository — skipping git/GitHub steps"
    SKIP_GH=true
fi
...
if [[ "$CURRENT_VERSION" != "$VERSION" || "$CURRENT_BUILD" != "$BUILD" ]]; then
    ...
    if in_git_repo; then
        git -C "$PROJECT_ROOT" add "$VERSION_CONFIG"
        git -C "$PROJECT_ROOT" commit -m "Release $VERSION ($BUILD)" >/dev/null
...
if git -C "$PROJECT_ROOT" rev-parse "$TAG" >/dev/null 2>&1; then
    ...
    git -C "$PROJECT_ROOT" tag -a "$TAG" -m "Release $VERSION ($BUILD)"
fi
git -C "$PROJECT_ROOT" push origin "$TAG" 2>/dev/null || git -C "$PROJECT_ROOT" push origin "$TAG"
